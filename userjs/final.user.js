(function (_w_) {
    function paternMatch(url, patern) {
        return new RegExp(`^${patern.replace(/[.+?^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*')}$`, 'i').test(url);
    }
    ...
    const base_url = "*://*.ai-augmented.com/api/jx-iresource/resource/query";
    const apis = {
        queryCourseResources: base_url + "CourseResources*",
        queryResource: base_url + "Resource*"
    };
    const mime = {
    ...
    const XhrFilter = (function (_w) {
        ...
        const getMatched = (url) => Array.from(filters.entries()).filter(entry => paternMatch(url, entry[0])).map(e => e[1]);
